...
console.log("🚀 vendor-ai-worker starting up", {
  ...
  env_preview: {
    ...
    VERTEX_PROJECT_ID: !!(process.env.VERTEX_PROJECT_ID || process.env.GCP_PROJECT)
  }
});
...
const PROJECT_ID =
  process.env.VERTEX_PROJECT_ID || process.env.GCP_PROJECT || process.env.GCP_PROJECT_ID;
const LOCATION = process.env.VERTEX_LOCATION || "us-central1";
const VISION_MODEL = process.env.VISION_MODEL_NAME; // e.g. "gemini-1.0-pro-vision"
const IMAGE_MODEL =
  process.env.IMAGE_MODEL_NAME || process.env.IMAGE_MODEL || "imagegeneration@006";
  ...
  if (Array.isArray(json.predictions)) {
    for (const p of json.predictions) {
      if (p.bytesBase64Encoded) candidates.push(`data:image/png;base64,${p.bytesBase64Encoded}`);
        ...
        candidates.push(p.image.startsWith("data:") ? p.image : `data:image/png;base64,${p.image}`);
    }
  }
  return candidates;
}
...
async function callImageToImageEnhance(imageDataUri, prompt, count = 1) {
  ...
  const body = {
    ...
    parameters: {
      sampleCount: clampSamples(count),
      aspectRatio: "1:1",
      temperature: 0.6,
      guidanceScale: 5.0
    }
...
app.post("/orchestrate", async (req, res) => {
  try {
    ...
    const inpaintedVariants =
      enhancedVariants.length > 0
        ? await callInpaintModel(
            enhancedVariants[0],
            "Replace background with a clean studio white background. Keep product unchanged.",
            1
          ).catch(() => [])
        : [];
