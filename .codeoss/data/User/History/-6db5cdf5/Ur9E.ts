...
serve(async (req) => {
  ...
  try {
    ...
    if (insertResult.insertErrors) {
      console.error("BigQuery insert error:", insertResult.insertErrors);
      return new Response(JSON.stringify({ error: insertResult.insertErrors }), {
        status: 500,
    ...
    // ✅ Normalize queryResult into array of objects
    const analytics: { date: string; total_sales: number; order_count: number }[] = [];
    if (queryResult.rows && queryResult.schema) {
      ...
      for (const row of queryResult.rows) {
        ...
        analytics.push({
          date: obj.date,
          total_sales: parseFloat(obj.total_sales),
          order_count: parseInt(obj.order_count),
        });
      }
    }
