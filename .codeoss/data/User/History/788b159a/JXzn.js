...
// --- The rest of your file remains unchanged (routes + helpers) ---
 ...
 * 3) call segmentation -> get mask base64
 * 4) call inpainting/background-edit (mask supplied) -> background-only edits
 * 5) composite product back onto edited background
 * 6) optional super-res
 * 7) compute fidelity checks
 * 8) upload accepted variants + mask to Supabase
 */
app.post("/orchestrate", async (req, res) => {
  try {
    ...
    // 3) Run segmentation to generate mask (base64 PNG mask)
    let maskDataUri;
    try {
      ...
      // Fallback: Try a conservative "no-edit" flow by returning minimal edits (tone/white-balance)
      return res.status(500).json({ error: "orchestration_failed", details: `Segmentation failed: ${String(err?.message ?? err)}` });
    }

    // 4) Use inpainting/instructed image edit to change only background (pass mask)
    ...
    // 5) For each variant, composite preserved product onto edited background, SR, QA, upload
    const uploadedUrls = [];
      ...
      try {
        ...
        if (SR_MODEL) {
          try {
            ...
            // Extract b64 and convert to buffer
...
async function callSegmentationModel(imageDataUri) {
  ...
  const url = `${REGION_BASE}/projects/${PROJECT_ID}/locations/${LOCATION}/publishers/google/models/${SEGMENT_MODEL}:predict`;
  // The exact predict body depends on the model flavor. We send image and ask for mask output.
  const body = {
    instances: [
      {
  ...
  const json = JSON.parse(text);
  // The return format varies by model. We attempt common patterns:
...
async function callInpaintModel(imageDataUri, maskDataUri, prompt, count = 1) {
  ...
  // Extract raw base64 for image and mask
  const imageBase64 = imageDataUri.replace(/^data:image\/\w+;base64,/, "");
  const maskBase64 = maskDataUri.replace(/^data:image\/\w+;base64,/, "");

  const url = `${REGION_BASE}/projects/${PROJECT_ID}/locations/${LOCATION}/publishers/google/models/${INPAINT_MODEL}:predict`;

  // This call shape assumes the model accepts image + mask; adapt as necessary for your Vertex model.
  const body = {
    instances: [
      {
        image: { imageBytes: imageBase64 },
...
// callSRModel: optional super-resolution model that returns data URI
async function callSRModel(imageDataUri) {
  if (!SR_MODEL) throw new Error("SR_MODEL_NAME not configured");
  const accessToken = await getAccessToken();
  const imageBase64 = imageDataUri.replace(/^data:image\/\w+;base64,/, "");

  const url = `${REGION_BASE}/projects/${PROJECT_ID}/locations/${LOCATION}/publishers/google/models/${SR_MODEL}:predict`;
  const body = {
...
// Composite: take original (product) and mask, and edited background; return PNG buffer
async function compositeProductOntoBackground(originalDataUri, maskDataUri, editedBackgroundDataUri) {
  ...
  // Normalize sizes: resize background to original dimensions
  ...
  // - product = original image where mask is white
  // - background = edited background where mask is black
  const productOnly = await sharp(origBuffer)
    .joinChannel(maskBuffer) // ensure mask set as alpha (works if mask is single-channel)
    .png()
    .toBuffer();

  // Create shadow: simple approach - offset mask, blur, darken, composite under product
...
// Simple fidelity checks: crop product from original and final using mask and compute MSE / PSNR / avg color delta
async function runFidelityChecks(originalDataUri, maskDataUri, finalBuffer) {
  try {
    ...
    // Resize everything to a manageable size for metrics (e.g., 512)
    const targetW = 512;
    ...
    const [origRaw, finalRaw, maskRaw] = await Promise.all([origImg.raw().toBuffer({ resolveWithObject: true }), finalImg.raw().toBuffer({ resolveWithObject: true }), maskImg.raw().toBuffer({ resolveWithObject: true })]);

    const origPixels = origRaw.data;
    const finalPixels = finalRaw.data;
    const maskPixels = maskRaw.data;
    const channels = origRaw.info.channels; // likely 3

    // compute MSE over product pixels (mask > 128)
    let sse = 0;
    let count = 0;
    let totalColorDelta = 0;
    for (let i = 0, p = 0; i < maskPixels.length; i += maskRaw.info.channels, p += channels) {
      const maskVal = maskPixels[i]; // single-channel mask
      if (maskVal > 128) {
        // product pixel — compute diff across channels
        for (let c = 0; c < 3; c++) {
          const d = origPixels[p + c] - finalPixels[p + c];
          sse += d * d;
        }
        // color delta heuristic: avg absolute channel difference
...
// Upload data URI buffer or buffer to Supabase storage
...
// upload base64/dataURI helper (kept for backward compat)
