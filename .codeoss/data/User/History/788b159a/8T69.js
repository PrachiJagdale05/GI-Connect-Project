// index.js - updated to support full resource strings for custom Vertex models (Cloud Run safe, ESM)
...
const SEGMENT_MODEL = process.env.SEGMENT_MODEL_NAME; // segmentation (custom full resource or publisher name)
const INPAINT_MODEL = process.env.INPAINT_MODEL_NAME; // inpainting (custom full resource or publisher name)
const SR_MODEL = process.env.SR_MODEL_NAME; // optional SR model (custom full resource or publisher name)
...
async function initializeRuntime() {
  ...
  // segmentation/inpaint/SR are optional — do not block server startup
  if (missing.length) {
    console.error("Missing required env vars:", missing.join(", "));
    // Keep server running but note that orchestration will fail until required vars are set.
    return;
  }
...
/* -------------------- Helper: build URL for publisher vs custom models -------------------- */
/**
 * modelEnvValue examples:
 *  - "gemini-2.0-flash-lite"              -> publisher google model
 *  - "imagegeneration@006"                -> publisher google image model
 *  - "projects/123/locations/us-central1/models/285744..." -> custom model resource
 *
 * method: "predict" | "generateContent" etc.
 */
function buildModelPredictUrl(modelEnvValue, method = "predict") {
  if (!modelEnvValue) throw new Error("Model env value not provided");
  // If already a full model resource path, use it directly
  if (modelEnvValue.startsWith("projects/")) {
    // example: projects/PROJECT/locations/LOCATION/models/MODEL_ID
    return `${REGION_BASE}/${modelEnvValue}:${method}`;
  }
  // otherwise assume it's a publisher model name (google)
  return `${REGION_BASE}/projects/${PROJECT_ID}/locations/${LOCATION}/publishers/google/models/${modelEnvValue}:${method}`;
}
  ...
  try {
    ...
    if (SEGMENT_MODEL) {
      try {
        ...
        // fail early - segmentation required for background edit flow
        return res.status(500).json({ error: "orchestration_failed", details: `Segmentation failed: ${String(err?.message ?? err)}` });
      }
    } else {
      // If segmentation not configured, we cannot do masked background edits — fallback to prompt-based generation
      console.warn("SEGMENT_MODEL_NAME not configured; skipping segmentation step");
      // You might choose to continue with full-image edits if desired (not implemented here)
      return res.status(400).json({ error: "segmentation_not_configured", details: "SEGMENT_MODEL_NAME env not set" });
    }

    // inpaint (optional)
    if (!INPAINT_MODEL) {
      console.warn("INPAINT_MODEL_NAME not configured; cannot perform masked inpainting");
      return res.status(400).json({ error: "inpaint_not_configured", details: "INPAINT_MODEL_NAME env not set" });
    }
    ...
    // composite, SR (optional), QA, upload
    const uploadedUrls = [];
...
Use the provided image and the product name: ${productName}`;
  ...
  const url = buildModelPredictUrl(VISION_MODEL, "generateContent"); // publisher model path expected
  const body = {
    contents: [
      {
        role: "user",
...
async function callSegmentationModel(imageDataUri) {
  ...
  const text = await resp.text();
  if (!resp.ok) {
    console.error("Segmentation request failed:", resp.status, text, "url:", url);
  ...
  if (maskB64) return `data:image/png;base64,${maskB64}`;

  // fallback: some models return a candidate with data URI
  const candidate = json?.candidates?.[0]?.content?.parts?.[0]?.text;
  if (candidate && candidate.startsWith("data:image")) return candidate;

  // try to find any data URI anywhere
  const textJoined = JSON.stringify(json);
  const m = textJoined.match(/data:image\/\w+;base64,[A-Za-z0-9+/=]+/);
  if (m) return m[0];
...
async function callInpaintModel(imageDataUri, maskDataUri, prompt, count = 1) {
  ...
  const text = await resp.text();
  if (!resp.ok) {
    console.error("Inpaint request failed:", resp.status, text, "url:", url);
...
async function callSRModel(imageDataUri) {
  ...
  const text = await resp.text();
  if (!resp.ok) {
    console.error("SR request failed:", resp.status, text, "url:", url);
...
/* -------------------- Image composite / QA / upload helpers -------------------- */
