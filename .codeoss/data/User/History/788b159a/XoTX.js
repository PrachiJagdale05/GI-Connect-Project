...
// It includes all fixes: role, mask structure, data normalization, and debugging.
...
async function callVisionToGenerateMask(imageBase64) {
    ...
    try {
        const cleanedString = rawMaskText.replace(/^data:image\/\w+;base64,/, "").replace(/```/g, '').replace('base64', '').trim();
...
app.post("/orchestrate", async (req, res) => {
  try {
    ...
    // --- THIS IS THE DEBUGGING LOG WE NEED TO SEE ---
