// index.js - Corrected and Final Working Version
...
console.log("vendor-ai-worker starting", { node: process.version });
...
const SEGMENTATION_MODEL = process.env.SEGMENTATION_MODEL_NAME || "multimodal-segmentation@001";
...
async function uploadBase64ToSupabase(base64OrDataUri, pathFilename) {
  ...
  const { error } = await supabase.storage.from(SUPABASE_BUCKET).upload(path, buffer, { contentType: `image/png`, upsert: true });
...
async function callVisionGenerate(imageBase64, productName) {
  ...
  return { product_name: parsed.product_name ?? productName, category: parsed.category ?? "Other", description: parsed.description ?? "", price: parsed.price ?? 0, stock: parsed.stock ?? 0 };
}
...
async function callSegmentationModel(imageBase64) {
    ...
    const body = { instances: [{ "image": { "bytesBase64Encoded": imageBase64 } }] };
    const resp = await fetch(url, { method: "POST", headers: { Authorization: `Bearer ${accessToken}`, "Content-Type": "application/json" }, body: JSON.stringify(body) });
    const text = await resp.text();
...
async function callInpaintingWithMask(imageBase64, maskBase64, prompt, sampleCount) {
    ...
    const body = {
        instances: [{ prompt: prompt, image: { bytesBase64Encoded: imageBase64 }, mask: { image: { bytesBase64Encoded: maskBase64 } } }],
...
app.post("/orchestrate", async (req, res) => {
  try {
    ...
    if (!incoming || !WORKER_SECRET || incoming !== WORKER_SECRET) {
    ...
    for (let i = 0; i < enhancedImages.length; i++) {
      ...
      } catch (err) { console.warn(`Upload failed for variant ${i}`, err?.message); }
    }
    if (!uploadedUrls.length) {
      return res.status(502).json({ error: "all_image_uploads_failed", vision: visionData });
    }
    const responsePayload = { ...visionData, price: Number(visionData.price) || 0, stock: Number(visionData.stock) || 0, generated_images: uploadedUrls };
