// index.js - vendor-ai-worker (Complete, Robust Mask & Inpaint Version)
...
// Use a powerful multimodal model for vision/masking tasks, e.g., gemini-1.5-pro-001 or gemini-1.5-flash-001
...
async function uploadBase64ToSupabase(base64OrDataUri, pathFilename) {
    ...
    const path = `${pathFilename}.png`; // Always save as png for consistency
...
/**
 * Calls Gemini to extract structured product data from an image.
 */
...
product_name, category, description (1-2 sentences), price (approx number), stock (approx int).
...
/**
 * STEP 1: Use a powerful vision model to generate a mask of the product.
 */
async function callVisionToGenerateMask(imageBase64) {
    ...
    const body = {
        contents: [{
            parts: [
                { text: `Based on the image, identify the primary product. Generate a black and white mask image where the entire product is solid white (#FFFFFF) and the background is solid black (#000000). The output should be only the image, with no extra text or explanation.` },
                { inline_data: { mime_type: "image/jpeg", data: imageBase64 } }
            ]
        }],
        generation_config: {
            // This is key: asking the model to respond with an image
    ...
    const text = await resp.text();
    if (!resp.ok) {
        throw new Error(`Mask generation failed: ${resp.status} ${text}`);
    }

    const json = JSON.parse(text);
    const maskBase64 = json.candidates[0]?.content?.parts[0]?.inline_data?.data;

    if (!maskBase64) {
        throw new Error("Vision model did not return a valid mask image.");
    }
    
    console.log("Successfully generated product mask.");
    return maskBase64;
}

/**
 * STEP 2: Use the image, the mask, and a prompt to perform inpainting.
 */
async function callInpaintingWithMask(imageBase64, maskBase64, prompt, sampleCount) {
    ...
    const body = {
        instances: [{
            ...
            mask: { bytesBase64Encoded: maskBase64 }
        }],
        parameters: {
            sampleCount: sampleCount,
            mode: "inpainting", // Explicitly set the mode
            guidanceScale: 9, 
            aspectRatio: "1:1"
        }
    };
    ...
    const text = await resp.text();
    if (!resp.ok) {
        throw new Error(`Inpainting failed: ${resp.status} ${text}`);
    }
...
app.post("/orchestrate", async (req, res) => {
  try {
    ...
    // Fetch the vendor image and encode it once
    ...
    // STEP 3: Inpaint using the generated mask
    ...
    // Return the successful response with metadata and images
