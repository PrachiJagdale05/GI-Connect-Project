// index.js - vendor-ai-worker (FIX 3: Corrected inpainting mask structure)
...
async function callVisionToGenerateMask(imageBase64) {
    ...
    if (!rawMaskText) {
        throw new Error("Vision model did not return any text for the mask.");
    }
    
    const maskBase64 = rawMaskText.replace(/```/g, '').replace('base64', '').trim();

    try {
        const buffer = Buffer.from(maskBase64, 'base64');
        if (buffer.length < 100) { 
            throw new Error("Received an invalid or empty mask string.");
        }
    } catch (e) {
        console.error("Failed to decode Base64 string from AI. Response was:", maskBase64);
        throw new Error("AI returned a non-Base64 string for the mask. Cannot proceed.");
    }

    console.log("Successfully generated and validated product mask as a Base64 string.");
    return maskBase64;
}
...
async function callInpaintingWithMask(imageBase64, maskBase64, prompt, sampleCount) {
    ...
    const body = {
        instances: [{
            ...
            // FIX: The mask's image data must be nested inside an "image" object.
            mask: { 
