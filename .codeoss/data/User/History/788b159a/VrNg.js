...
// crash safety
process.on("uncaughtException", (err) => console.error("uncaughtException:", err?.stack || String(err)));
process.on("unhandledRejection", (reason) => console.error("unhandledRejection:", reason?.stack || String(reason)));
...
app.post("/orchestrate", async (req, res) => {
  try {
    ...
    // ---- ENHANCEMENT STEP ----
    try {
      rawImages = await enhanceImagesWithInpainting(rawImages);
    } catch (e) {
      console.warn("Image enhancement failed, using originals:", e.message);
    }
...
// --- NEW: Enhancement helper ---
async function callImageInpainting(base64Image, prompt = "Enhance details, improve realism, better lighting") {
  const accessToken = await getAccessToken();
  ...
  const body = {
    instances: [{
      prompt,
      image: { bytesBase64Encoded: base64Image.replace(/^data:image\/\w+;base64,/, "") }
    }],
...
async function enhanceImagesWithInpainting(images) {
  const results = [];
  for (const img of images) {
    try {
      results.push(await callImageInpainting(img));
    } catch (e) {
      console.warn("Enhancement for one image failed, keeping original:", e.message);
      results.push(img);
    }
  }
  return results;
