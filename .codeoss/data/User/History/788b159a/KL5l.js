...
const MAX_IMAGES = 4; // clamp to 4 (Imagen limit)
  ...
  try {
    ...
    try {
      ...
      return res.status(500).json({
        ...
        details: `Vision model failed: ${err.message}`
      });
    }

    // 2. Construct enhancement prompt
    const prompt = `Enhance this product photo while keeping the product identical. 
Improve quality, sharpness, and lighting. Make it look professional and suitable for e-commerce.
Product: ${visionResult.product_name}.`;

    // 3. Image generation
    let rawImages;
    try {
      rawImages = await callImageGeneration(imageUrl, prompt, MAX_IMAGES);
    } catch (err) {
      console.error("callImageGeneration failed:", err);
      return res.status(500).json({
        error: "orchestration_failed",
        details: `Image generation failed: ${err.message}`
      });
    }
...
async function callImageGeneration(imageUrl, prompt, count = 1) {
